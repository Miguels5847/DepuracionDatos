...
def check_db_records():
    ...
    try:
        ...
        # Verificar tabla sube_alim (origen)
        cursor.execute("SELECT COUNT(*) FROM sube_alim")
        source_count = cursor.fetchone()[0]
        logger.info(f"Registros en sube_alim (origen): {source_count}")
        
        # Verificar tabla sube_alim_processed (destino)
        cursor.execute("SELECT COUNT(*) FROM sube_alim_processed")
        dest_count = cursor.fetchone()[0]
        logger.info(f"Registros en sube_alim_processed (destino): {dest_count}")
        
        # Obtener algunos ejemplos
        cursor.execute("SELECT * FROM sube_alim_processed LIMIT 3")
