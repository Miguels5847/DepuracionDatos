...
# IDs de procesadores - Grupo Productor - ACTUALIZADO
QUERY_DB_TABLE_PROCESSOR_ID = "132ed7b1-19c7-3fc1-0e0f-eda7c207604a"  # QueryDatabaseTable
CONVERT_RECORD_ID = "2b6dd61e-1514-3bf1-6452-eb5a13c8d582"
SPLIT_JSON_ID = "4ada6bd5-c816-3289-12d2-535853ec3d7f"
MERGE_CONTENT_ID = "e2e4fc91-275d-388b-277d-1d074573e7b1"
PUBLISH_KAFKA_ID = "8d38053b-33b2-3de6-e2c7-12f166741b0b"
PUT_DATABASE_RECORD_ID = "17552b65-9986-39ab-6bb7-1947b3f6f353"

# IDs de procesadores - Grupo Consumidor - ACTUALIZADO
CONSUME_KAFKA_PROCESSOR_ID = "f0466d14-22be-35b2-c2d8-66e8dacba71d"
EVALUATE_JSON_PATH_ID = "4d496c11-afa8-3b5f-5a3e-46f11ccaeb3d"
REPLACE_TEXT_ID = "49f31676-d89f-365f-343e-a1ea1b00f0c0"
PUT_DB_PROCESSOR_ID = "770949d8-d3f2-30b1-2973-d8496c08a2ea"

# Función para inicializar el gestor NiFi y configurar los IDs
def inicializar_gestor_nifi():
    """
    Inicializa el gestor NiFi y configura los IDs de los procesadores.
    Usa una estrategia robusta para resolver IDs o usar valores hardcodeados como fallback.
    """
    global QUERY_DB_TABLE_PROCESSOR_ID, CONSUME_KAFKA_PROCESSOR_ID
    global CONVERT_RECORD_ID, SPLIT_JSON_ID, MERGE_CONTENT_ID
    global PUBLISH_KAFKA_ID, PUT_DATABASE_RECORD_ID
    global EVALUATE_JSON_PATH_ID, REPLACE_TEXT_ID, PUT_DB_PROCESSOR_ID
    global PRODUCER_GROUP_ID, CONSUMER_GROUP_ID
    global gestor_nifi
    
    # Valores hardcodeados conocidos que funcionan (fallback)
    fallback_ids = {
        "query_db_table": "a6ee2f4e-edbc-38f9-a152-b0a19044d4a9",
        "convert_record": "47082c97-43ab-3005-31b7-3a45868f9d4f",
        "split_json": "610f8ec8-efbb-393a-67d7-2e1f6d72cdd2",
        "merge_content": "c78203d6-ff06-3657-6bc9-e78f115b5426",
        "publish_kafka": "5f56daf3-3075-3d6a-53b0-4f1106a9e790",
        "put_database_record": "10d89865-0196-1000-3eaf-c624cd671d97",
        "consume_kafka": "e70f09af-1320-30d9-d640-9b0cb7e39f8a",
        "evaluate_json_path": "ef0e8535-8d18-3ee2-e0a7-38281ba8fe2a",
        "replace_text": "398ab46d-50e6-3101-c4dc-fd605138661c",
        "put_db": "db428504-4b50-3703-22eb-367ccbf3b295"
    }
    
    # Grupos
    fallback_grupos = {
        "producer_group": "3f037cd2-5493-35fd-6a78-309aa23e598a",
        "consumer_group": "6364f7c9-07f1-394d-021c-db2529cca44c"
    }
    
    try:
        print("🔄 Inicializando gestor NiFi...")
        
        # Verificar si el proxy está disponible antes de continuar
        response = requests.get(f"{MINI_PROXY_URL}/api/system/status", timeout=HTTP_TIMEOUT)
        
        # Diccionario para almacenar los IDs encontrados
        processor_ids = {}
        
        # 1. ESTRATEGIA PRINCIPAL: Usar el sistema de descubrimiento si está disponible
        if nifi_discovery_available:
            print("🔍 Usando NiFiDiscovery para resolver procesadores...")
            nifi_discovery = NiFiDiscovery(proxy_url=MINI_PROXY_URL, timeout=HTTP_TIMEOUT)
            
            # Mapeo de términos de búsqueda para cada tipo de procesador
            search_terms_map = {
                "query_db_table": ["QueryDatabaseTable", "P.1-QueryDatabaseTable", "Query"],
                ...
                "publish_kafka": ["PublishKafka", "P.3-PublishKafka", "PublishKafkaRecord"],
                "put_database_record": ["PutDatabaseRecord", "P.4-PutDatabaseRecord"],
                "consume_kafka": ["ConsumeKafka", "C.1-ConsumeKafka", "ConsumeKafkaRecord"],
            ...
            # Resolver cada procesador
            for key, search_terms in search_terms_map.items():
                processor_ids[key] = nifi_discovery.get_processor_id(
                    key, 
                    search_terms, 
                    fallback_ids.get(key)
                )
        
        # 2. ESTRATEGIA ALTERNATIVA: Búsqueda directa por API
        if not nifi_discovery_available or len(processor_ids) < len(fallback_ids):
            try:
                print("🔍 Buscando procesadores usando endpoint de búsqueda...")
                # Lista de términos de búsqueda para cada tipo de procesador
                search_terms = {
                    "query_db_table": ["QueryDatabaseTable", "Query"],
                    "convert_record": ["ConvertRecord"],
                    "split_json": ["SplitJson"],
                    "merge_content": ["MergeContent"],
                    "publish_kafka": ["PublishKafka"],
                    "put_database_record": ["PutDatabaseRecord"],
                    "consume_kafka": ["ConsumeKafka"],
                    "evaluate_json_path": ["EvaluateJsonPath"],
                    "replace_text": ["ReplaceText"],
                    "put_db": ["PutDatabase"]
                }
                
                for key, terms in search_terms.items():
                    # Si ya tenemos este ID, omitir
                    if key in processor_ids:
                        continue
                        
                    for term in terms:
                        search_response = requests.get(
                            f"{MINI_PROXY_URL}/api/flow/search-results?q={term}",
                            timeout=HTTP_TIMEOUT
                        )
                        
                        if search_response.status_code == 200:
                            search_data = search_response.json()
                            if "searchResultsDTO" in search_data and "processorResults" in search_data["searchResultsDTO"]:
                                results = search_data["searchResultsDTO"]["processorResults"]
                                if results:
                                    processor_ids[key] = results[0].get("id")
                                    print(f"✅ Encontrado ID para {key}: {processor_ids[key]}")
                                    break
                
            except Exception as search_error:
                print(f"⚠️ Error en búsqueda directa: {str(search_error)}")
        
        # 3. ESTRATEGIA DE GESTOR: Usar el GestorNiFi si está disponible
        if gestor_nifi_importado:
            try:
                print("🔄 Intentando crear instancia de GestorNiFi...")
                gestor = GestorNiFi(proxy_url=MINI_PROXY_URL, timeout=HTTP_TIMEOUT)
                gestor.verificar_disponibilidad()
                gestor.resolver_ids_procesadores()
                
                # Intentar obtener IDs del gestor
                for key, fallback in fallback_ids.items():
                    # Si ya tenemos este ID, omitir
                    if key in processor_ids:
                        continue
                        
                    attr_name = f"{key}_processor_id"
                    if hasattr(gestor, attr_name) and getattr(gestor, attr_name):
                        processor_ids[key] = getattr(gestor, attr_name)
                        print(f"✅ ID para {key} resuelto por gestor: {processor_ids[key]}")
                
                # Obtener IDs de grupos
                if hasattr(gestor, 'producer_group_id') and gestor.producer_group_id:
                    fallback_grupos["producer_group"] = gestor.producer_group_id
                if hasattr(gestor, 'consumer_group_id') and gestor.consumer_group_id:
                    fallback_grupos["consumer_group"] = gestor.consumer_group_id
                
                # Almacenar el gestor para uso posterior
                gestor_nifi = gestor
                
            except Exception as gestor_error:
                print(f"⚠️ Error con GestorNiFi: {str(gestor_error)}")
        
        # 4. ASIGNACIÓN FINAL: Asignar IDs finales, usando los resueltos o fallbacks
        QUERY_DB_TABLE_PROCESSOR_ID = processor_ids.get("query_db_table", fallback_ids["query_db_table"])
        CONVERT_RECORD_ID = processor_ids.get("convert_record", fallback_ids["convert_record"])
        SPLIT_JSON_ID = processor_ids.get("split_json", fallback_ids["split_json"])
        MERGE_CONTENT_ID = processor_ids.get("merge_content", fallback_ids["merge_content"])
        PUBLISH_KAFKA_ID = processor_ids.get("publish_kafka", fallback_ids["publish_kafka"])
        PUT_DATABASE_RECORD_ID = processor_ids.get("put_database_record", fallback_ids["put_database_record"])
        CONSUME_KAFKA_PROCESSOR_ID = processor_ids.get("consume_kafka", fallback_ids["consume_kafka"])
        EVALUATE_JSON_PATH_ID = processor_ids.get("evaluate_json_path", fallback_ids["evaluate_json_path"])
        REPLACE_TEXT_ID = processor_ids.get("replace_text", fallback_ids["replace_text"])
        PUT_DB_PROCESSOR_ID = processor_ids.get("put_db", fallback_ids["put_db"])
        
        # Asignar IDs de grupos
        PRODUCER_GROUP_ID = fallback_grupos["producer_group"]
        CONSUMER_GROUP_ID = fallback_grupos["consumer_group"]
        
        # 5. PERSISTENCIA: Guardar los IDs encontrados para futuras ejecuciones
        if nifi_discovery_available:
            for key, processor_id in processor_ids.items():
                nifi_discovery._save_id(key, processor_id)
        
        # Mostrar resumen de los IDs asignados
        print("\n✅ RESUMEN DE IDs CONFIGURADOS:")
        print(f"PRODUCER_GROUP_ID: {PRODUCER_GROUP_ID}")
        print(f"CONSUMER_GROUP_ID: {CONSUMER_GROUP_ID}")
        print(f"QUERY_DB_TABLE_PROCESSOR_ID: {QUERY_DB_TABLE_PROCESSOR_ID}")
        print(f"CONSUME_KAFKA_PROCESSOR_ID: {CONSUME_KAFKA_PROCESSOR_ID}")
        
        # Retornar el gestor si se creó correctamente
        return gestor_nifi
        
    except Exception as e:
        print(f"⚠️ Error inicializando gestor NiFi: {str(e)}")
        
        # Usar valores hardcodeados como último recurso
        QUERY_DB_TABLE_PROCESSOR_ID = fallback_ids["query_db_table"]
        CONVERT_RECORD_ID = fallback_ids["convert_record"]
        SPLIT_JSON_ID = fallback_ids["split_json"]
        MERGE_CONTENT_ID = fallback_ids["merge_content"]
        PUBLISH_KAFKA_ID = fallback_ids["publish_kafka"]
        PUT_DATABASE_RECORD_ID = fallback_ids["put_database_record"]
        CONSUME_KAFKA_PROCESSOR_ID = fallback_ids["consume_kafka"]
        EVALUATE_JSON_PATH_ID = fallback_ids["evaluate_json_path"]
        REPLACE_TEXT_ID = fallback_ids["replace_text"]
        PUT_DB_PROCESSOR_ID = fallback_ids["put_db"]
        PRODUCER_GROUP_ID = fallback_grupos["producer_group"]
        CONSUMER_GROUP_ID = fallback_grupos["consumer_group"]
        
        print("⚠️ Usando IDs hardcodeados como fallback")
        return None
...
def probar_inicializacion_procesadores(duracion_segundos=1):
    """
    Prueba diagnóstica simplificada que solo verifica el estado de los procesadores
    sin iniciarlos, para evitar cualquier procesamiento de datos.
    
    Args:
        duracion_segundos (int): Parámetro mantenido por compatibilidad pero no usado
    
    Returns:
        dict: Resumen del estado de los procesadores
    """
    try:
        global gestor_nifi
        resumen = {
            "grupo_productor": {
                "id": PRODUCER_GROUP_ID,
                "procesadores": []
            },
            "grupo_consumidor": {
        ...
        # Lista completa de todos los procesadores con información detallada
        procesadores_detallados = [
            # Grupo productor
            {
                "id": QUERY_DB_TABLE_PROCESSOR_ID,
                "nombre": "QueryDatabaseTable",
                "tipo": "org.apache.nifi.processors.standard.QueryDatabaseTable",
                "grupo": "productor",
                "descripcion": "Consulta la tabla potencia_dep para extraer datos"
            },
            {
                "id": CONVERT_RECORD_ID,
                "nombre": "ConvertRecord",
                "tipo": "org.apache.nifi.processors.standard.ConvertRecord",
                "grupo": "productor",
                "descripcion": "Convierte los resultados a formato JSON"
            },
            {
                "id": SPLIT_JSON_ID,
                "nombre": "SplitJson",
                "tipo": "org.apache.nifi.processors.standard.SplitJson",
                "grupo": "productor",
                "descripcion": "Divide el JSON en registros individuales"
            },
            {
                "id": MERGE_CONTENT_ID,
                "nombre": "MergeContent",
                "tipo": "org.apache.nifi.processors.standard.MergeContent",
                "grupo": "productor",
                "descripcion": "Agrupa registros para envío eficiente"
            },
            {
                "id": PUBLISH_KAFKA_ID,
                "nombre": "PublishKafka",
                "tipo": "org.apache.nifi.processors.kafka.PublishKafka",
                "grupo": "productor",
                "descripcion": "Publica datos al topic Kafka configurado"
            },
            {
                "id": PUT_DATABASE_RECORD_ID,
                "nombre": "PutDatabaseRecord",
                "tipo": "org.apache.nifi.processors.standard.PutDatabaseRecord",
                "grupo": "productor",
                "descripcion": "Guarda datos en tabla datos_enviados"
            },
            # Grupo consumidor
            {
                "id": CONSUME_KAFKA_PROCESSOR_ID,
                "nombre": "ConsumeKafka",
                "tipo": "org.apache.nifi.processors.kafka.ConsumeKafka",
                "grupo": "consumidor",
                "descripcion": "Consume datos del topic Kafka configurado"
            },
            {
                "id": EVALUATE_JSON_PATH_ID,
                "nombre": "EvaluateJsonPath",
                "tipo": "org.apache.nifi.processors.standard.EvaluateJsonPath",
                "grupo": "consumidor",
                "descripcion": "Extrae campos del JSON para procesamiento"
            },
            {
                "id": REPLACE_TEXT_ID,
                "nombre": "ReplaceText",
                "tipo": "org.apache.nifi.processors.standard.ReplaceText",
                "grupo": "consumidor",
                "descripcion": "Transforma datos para inserción en BD"
            },
            {
                "id": PUT_DB_PROCESSOR_ID,
                "nombre": "PutDatabaseRecord",
                "tipo": "org.apache.nifi.processors.standard.PutDatabaseRecord",
                "grupo": "consumidor",
                "descripcion": "Guarda datos procesados en potencia_dep_processed"
            }
        ]

        print("🔍 INICIANDO PRUEBA DE PROCESADORES - MODO DIAGNÓSTICO")
        print("=" * 70)
        
        # 1. Verificar que todos los procesadores estén detenidos primero
        print("⏹️ Asegurando que todos los procesadores estén detenidos...")
        stop_all_processors_individually()
        time.sleep(3)
        
        # 2. Verificar y mostrar estado actual de cada procesador
        print("\n📋 ESTADO INICIAL DE PROCESADORES:")
        print("-" * 70)
        print(f"{'ID PROCESADOR':<36} | {'NOMBRE':<20} | {'GRUPO':<10} | {'ESTADO':<10}")
        print("-" * 70)
        
        for proc in procesadores_detallados:
            estado_actual = obtener_estado_procesador(proc["id"])
            estado = estado_actual.get("component", {}).get("state", "DESCONOCIDO")
            print(f"{proc['id']:<36} | {proc['nombre']:<20} | {proc['grupo']:<10} | {estado:<10}")
            
            # Guardar en resumen
            if proc['grupo'] == 'productor':
                resumen["grupo_productor"]["procesadores"].append({
                    "id": proc['id'],
                    "nombre": proc['nombre'],
                    "estado": estado
                })
            else:
                resumen["grupo_consumidor"]["procesadores"].append({
                    "id": proc['id'],
                    "nombre": proc['nombre'],
                    "estado": estado
                })
        
        # Esperar el tiempo especificado para mostrar consistencia con la versión anterior
        print(f"\n⏱️ Esperando {duracion_segundos} segundo(s) para verificación final...")
        time.sleep(duracion_segundos)
        
        # 3. Verificar estado final (debería ser el mismo que el inicial ya que no iniciamos nada)
        print("\n📋 ESTADO FINAL DE PROCESADORES:")
        print("-" * 70)
        print(f"{'ID PROCESADOR':<36} | {'NOMBRE':<20} | {'GRUPO':<10} | {'ESTADO':<10}")
        print("-" * 70)
        ...
        print(f"❌ Error en prueba de inicialización: {str(e)}")
        traceback.print_exc()
        return {"estado": "fallido", "error": str(e)}

def verificar_registro_ids():
    """Verifica si los IDs se resolvieron correctamente y los registra en Variables Airflow"""
    try:
        processor_ids = {
            "query_db_table": QUERY_DB_TABLE_PROCESSOR_ID,
            "convert_record": CONVERT_RECORD_ID,
            "split_json": SPLIT_JSON_ID,
            "merge_content": MERGE_CONTENT_ID,
            "publish_kafka": PUBLISH_KAFKA_ID,
            "put_database_record": PUT_DATABASE_RECORD_ID,
            "consume_kafka": CONSUME_KAFKA_PROCESSOR_ID,
            "evaluate_json_path": EVALUATE_JSON_PATH_ID,
            "replace_text": REPLACE_TEXT_ID,
            "put_db": PUT_DB_PROCESSOR_ID
        }
        
        # Grupos
        group_ids = {
            "producer_group": PRODUCER_GROUP_ID,
            "consumer_group": CONSUMER_GROUP_ID
        }
        
        # Verificar cada ID contra la API para asegurar que son válidos
        verified_ids = {}
        
        for name, proc_id in processor_ids.items():
            print(f"Verificando procesador '{name}' con ID: {proc_id}")
            
            response = requests.get(
                f"{MINI_PROXY_URL}/api/processors/{proc_id}",
                timeout=HTTP_TIMEOUT
            )
            
            if response.status_code == 200:
                proc_data = response.json()
                proc_name = proc_data.get("component", {}).get("name", "Unknown")
                proc_type = proc_data.get("component", {}).get("type", "Unknown")
                ...
                print(f"✅ {name}: {proc_id} ({proc_name}) - VERIFICADO")
                
                # Si tenemos el módulo NiFiDiscovery, guardar en caché local y variables
                if nifi_discovery_available:
                    try:
                        nifi_discovery = NiFiDiscovery(proxy_url=MINI_PROXY_URL, timeout=HTTP_TIMEOUT)
                        nifi_discovery._save_id(name, proc_id)
                    except Exception as e:
                        print(f"⚠️ Error guardando ID en caché: {str(e)}")
            else:
                verified_ids[name] = {
                    "id": proc_id,
                ...
                print(f"⚠️ {name}: {proc_id} - NO VERIFICADO (Error: {response.status_code})")
        
        # Verificar grupos
        for name, group_id in group_ids.items():
            # Los grupos no tienen un endpoint simple de verificación en el mini-proxy
            verified_ids[name] = {
                "id": group_id,
                "verified": "assumed"  # No hay forma fácil de verificarlo con el mini-proxy
            }
        
        print("\n📋 RESUMEN DE VERIFICACIÓN DE IDS:")
        print("-" * 70)
        print(f"{'PROCESADOR':<20} | {'ID':<36} | {'ESTADO':<10}")
        print("-" * 70)
        
        for name, info in verified_ids.items():
            estado = "✅ OK" if info.get("verified") == True else "⚠️ ERROR" if info.get("verified") == False else "⚠️ SIN VERIFICAR"
            print(f"{name:<20} | {info['id']:<36} | {estado:<10}")
        
        return verified_ids
            
    except Exception as e:
        print(f"❌ Error en verificación de IDs: {str(e)}")
...
def obtener_estado_procesador(id_procesador):
    ...
    try:
        # Si no hay ID o es inválido, retornar error temprano
        if not id_procesador or len(id_procesador) < 10:  # IDs de NiFi son largos
            return {
                "error": f"ID de procesador inválido: {id_procesador}",
                "component": {"state": "DESCONOCIDO"}
            }
            
        # Usar el endpoint correcto del mini-proxy
        response = requests.get(
        ...
        if response.status_code == 200:
            ...
            # Si no se encuentra, probablemente el ID ha cambiado
            print(f"⚠️ Procesador no encontrado: {id_procesador} (posible cambio de ID)")
            
            # Intentar redescubrir usando endpoint alternativo
            try:
                alt_response = requests.get(
                    f"{MINI_PROXY_URL}/api/system/status?detail=advanced",
                    timeout=HTTP_TIMEOUT
                )
                
                if alt_response.status_code == 200:
                    print("✅ El sistema está en línea, pero el procesador no se encontró")
            except:
                print("⚠️ No se pudo verificar el estado del sistema")
            ...
            print(f"⚠️ Error al obtener estado del procesador: {response.status_code}")
            print(f"Respuesta: {response.text[:200]}")
            return {
                "error": f"Error al obtener estado del procesador: {response.status_code}",
                "raw_response": response.text[:200],
        ...
        print(f"❌ Error al consultar procesador: {str(e)}")
        return {
            "error": f"Error al consultar procesador: {str(e)}",
            "component": {"state": "DESCONOCIDO"}
        }
...
def detener_procesador(id_procesador):
    ...
    try:
        ...
        # Preparamos los datos para actualizar el estado - CORREGIDO
        update_data = {
            "revision": revision,
            "state": "STOPPED",
        ...
        # Enviamos la solicitud para cambiar el estado - CORREGIDO ENDPOINT
        response = requests.put(
            f"{MINI_PROXY_URL}/api/processors/{id_procesador}/run-status",
            json=update_data,
            timeout=HTTP_TIMEOUT,
        )
...
def iniciar_procesador_con_reintento(processor_id, processor_name, max_retries=5):
    ...
    for intento in range(1, max_retries + 1):
        try:
            ...
            # Enviar solicitud para cambiar estado - CORREGIDO ENDPOINT
            response = requests.put(
                f"{MINI_PROXY_URL}/api/processors/{processor_id}/run-status",
                json=update_data,
                timeout=HTTP_TIMEOUT,
            )
...
def stop_all_processors_individually():
    """Detiene todos los procesadores uno por uno usando el mini-proxy"""
    try:
        global gestor_nifi
        
        if gestor_nifi:
            print("📌 Usando gestor NiFi para detener procesadores...")
            return gestor_nifi.detener_todos_procesadores()
        
        # Si no hay gestor, usar el enfoque basado en IDs
        ...
        for processor in producer_processors:
            # Obtener estado actual y revisión
            response = requests.get(
                f"{MINI_PROXY_URL}/api/processors/{processor['id']}",
                timeout=HTTP_TIMEOUT
            )
            
            if response.status_code == 200:
                data = response.json()
                current_state = data.get("component", {}).get("state", "UNKNOWN")
                print(f"Estado actual del procesador {processor['id']} para detener: {current_state}")
                
                # Solo detener si no está ya detenido
                if current_state not in ["STOPPED", "DISABLED"]:
                    revision = data.get("revision", {})
                    
                    # Preparar payload para detener - CORREGIDO
                    update_data = {
                        "revision": revision,
                        "state": "STOPPED",
                        "disconnectedNodeAcknowledged": True
                    ...
                    # Usar el endpoint correcto para detener
                    stop_response = requests.put(
                        f"{MINI_PROXY_URL}/api/processors/{processor['id']}/run-status",
                        json=update_data,
                        timeout=HTTP_TIMEOUT
                    )
                    
                    if stop_response.status_code in [200, 201, 202]:
                        print(f"✅ Procesador {processor['name']} detenido correctamente")
                    else:
                        print(f"⚠️ Error al detener {processor['name']}: {stop_response.status_code}")
        
        # Hacer lo mismo para procesadores del consumidor
        ...
        for processor in consumer_processors:
            # Código similar para detener procesadores del consumidor - CORREGIDO DE IGUAL MANERA
            response = requests.get(
                f"{MINI_PROXY_URL}/api/processors/{processor['id']}",
                timeout=HTTP_TIMEOUT
            )
        ...
        print(f"❌ Error deteniendo procesadores: {str(e)}")
        return False
...
def configurar_topic_procesador(procesador_id, nombre_propiedad, valor):
    """Configura un topic en un procesador teniendo en cuenta las diferencias entre procesadores"""
    try:
        global gestor_nifi
        
        if gestor_nifi:
            return gestor_nifi.configurar_topic_kafka(procesador_id, valor)
        
        # Si no hay gestor, usar el enfoque basado en API directo
        # Obtener detalles del procesador
        response = requests.get(
            f"{MINI_PROXY_URL}/api/processors/{procesador_id}",
            timeout=HTTP_TIMEOUT
        )
        
        if response.status_code != 200:
            print(f"Error al obtener detalles del procesador: {response.status_code}")
            return False
        
        datos = response.json()
        
        # Obtener component type para determinar qué propiedad usar
        processor_type = datos.get("component", {}).get("type", "")
        
        # Detener procesador si está ejecutándose
        estado_actual = datos.get("component", {}).get("state")
        if estado_actual == "RUNNING":
            print(f"Estado actual del procesador {procesador_id} para detener: {estado_actual}")
            detener_procesador(procesador_id)
        
        # Actualizar configuración
        config = datos.get("component", {}).get("config", {})
        properties = config.get("properties", {})
        
        # Usar la propiedad correcta según el tipo de procesador
        if "ConsumeKafka" in processor_type:
            # ConsumeKafka usa "Topics", no "Topic Name"
            print(f"Configurando Topics para ConsumeKafka: {valor}")
            properties["Topics"] = valor
        elif "PublishKafka" in processor_type:
            # PublishKafka usa "Topic Name"
            print(f"Configurando Topic Name para PublishKafka: {valor}")
            properties["Topic Name"] = valor
        else:
            # Caso genérico, usar el nombre proporcionado
            print(f"Configurando {nombre_propiedad} para procesador genérico: {valor}")
            properties[nombre_propiedad] = valor
        ...
        if update_response.status_code in [200, 201]:
            print(f"✅ Topic actualizado correctamente a {valor} en {procesador_id}")
            return True
        else:
            print(f"Error actualizando topic: {update_response.status_code}")
            return False
    
    except Exception as e:
        print(f"Error configurando topic: {str(e)}")
        return False
...
def limpiar_estado_procesador(processor_id):
    ...
    try:
        ...
        # Usando el endpoint correcto del mini-proxy
        response = requests.post(
            f"{MINI_PROXY_URL}/api/processors/{processor_id}/state/clear",
            timeout=HTTP_TIMEOUT
        )
        ...
        if response.status_code in [200, 201, 202, 204]:
            ...
            print(f"❌ Error limpiando estado: {response.status_code}, {response.text[:100]}")
            return False
...
def clear_processor_state_and_empty_all_queues():
    ...
    try:
        ...
        # 2. Limpiar estado de los procesadores principales usando el endpoint correcto
        for processor_id in [QUERY_DB_TABLE_PROCESSOR_ID, CONSUME_KAFKA_PROCESSOR_ID]:
            try:
                # Usar el endpoint explícito de state/clear
                response = requests.post(
                    f"{MINI_PROXY_URL}/api/processors/{processor_id}/state/clear",
                    timeout=HTTP_TIMEOUT
                )
                ...
                if response.status_code in [200, 201, 202, 204]:
                    print(f"✅ Estado de procesador {processor_id} limpiado")
                else:
                    print(f"⚠️ Error limpiando estado de {processor_id}: {response.status_code}")
                    
                    # Intentar endpoint alternativo si el primero falla
                    alt_response = requests.delete(
                        f"{MINI_PROXY_URL}/api/processors/{processor_id}/sql",
                        timeout=HTTP_TIMEOUT
                    )
                    if alt_response.status_code in [200, 202, 204]:
                        print(f"✅ Estado limpiado mediante endpoint alternativo")
            except Exception as e:
                print(f"⚠️ Error limpiando estado de {processor_id}: {str(e)}")

        # 3. Usar el endpoint unificado para limpiar todas las colas
        try:
            print("🧹 Limpiando todas las colas usando endpoint unificado...")
            clean_response = requests.post(
                f"{MINI_PROXY_URL}/api/clean-all-queues",
                timeout=HTTP_TIMEOUT
            )
            
            if clean_response.status_code in [200, 201, 202, 204]:
                print("✅ Limpieza unificada de colas completada")
                print(f"Detalles: {clean_response.text[:200]}")
            else:
                print(f"⚠️ Error en limpieza unificada: {clean_response.status_code}")
        except Exception as e:
            print(f"⚠️ Error con endpoint unificado: {str(e)}")
            
            # Fallback: Listar y vaciar colas individualmente
            try:
                # Primero, buscar información sobre colas disponibles
                print("🔍 Buscando información sobre colas...")
                info_response = requests.get(
                    f"{MINI_PROXY_URL}/api/system/status?detail=advanced&checks=queues",
                    timeout=HTTP_TIMEOUT
                )
                
                if info_response.status_code == 200:
                    info_data = info_response.json()
                    if "connections" in info_data:
                        connections = info_data["connections"]
                        print(f"📋 Se encontraron {len(connections)} conexiones")
                        
                        # Intentar limpiar cada conexión
                        for conn in connections:
                            conn_id = conn.get("id")
                            if conn_id:
                                try:
                                    requests.post(
                                        f"{MINI_PROXY_URL}/api/connections/{conn_id}/drop-requests",
                                        json={"disconnectedNodeAcknowledged": True},
                                        timeout=HTTP_TIMEOUT
                                    )
                                    print(f"✅ Cola {conn_id} limpiada")
                                except:
                                    print(f"⚠️ Error limpiando cola {conn_id}")
            except:
                print("⚠️ No se pudieron limpiar colas mediante el método alternativo")

        time.sleep(5)  # Allow time for cleanup operations to complete
        print("✅ Operaciones de limpieza completadas")
        return True

    except Exception as e:
        print(f"⚠️ Error general en limpieza: {str(e)}")
        return False

def reinicializar_ids_conocidos():
    """
    Reinicializa manualmente los IDs de procesadores y grupos basados 
    en búsqueda directa sin depender de los sistemas de descubrimiento
    """
    global QUERY_DB_TABLE_PROCESSOR_ID, CONSUME_KAFKA_PROCESSOR_ID
    global CONVERT_RECORD_ID, SPLIT_JSON_ID, MERGE_CONTENT_ID
    global PUBLISH_KAFKA_ID, PUT_DATABASE_RECORD_ID
    ...
    print("♻️ REINICIALIZANDO IDs DE PROCESADORES...")
    
    try:
        # Usar el endpoint de búsqueda para cada procesador clave
        procesadores_claves = [
            {"name": "QueryDatabaseTable", "search": "QueryDatabaseTable", "var": "QUERY_DB_TABLE_PROCESSOR_ID"},
            {"name": "ConvertRecord", "search": "ConvertRecord", "var": "CONVERT_RECORD_ID"},
            {"name": "SplitJson", "search": "SplitJson", "var": "SPLIT_JSON_ID"},
            {"name": "MergeContent", "search": "MergeContent", "var": "MERGE_CONTENT_ID"},
            {"name": "PublishKafka", "search": "PublishKafka", "var": "PUBLISH_KAFKA_ID"},
            {"name": "PutDatabaseRecord", "search": "PutDatabaseRecord", "var": "PUT_DATABASE_RECORD_ID"},
            {"name": "ConsumeKafka", "search": "ConsumeKafka", "var": "CONSUME_KAFKA_PROCESSOR_ID"},
            {"name": "EvaluateJsonPath", "search": "EvaluateJsonPath", "var": "EVALUATE_JSON_PATH_ID"},
            {"name": "ReplaceText", "search": "ReplaceText", "var": "REPLACE_TEXT_ID"}
        ]
        
        encontrados = 0
        
        for proc in procesadores_claves:
            try:
                print(f"🔍 Buscando {proc['name']}...")
                response = requests.get(
                    f"{MINI_PROXY_URL}/api/flow/search-results?q={proc['search']}",
                    timeout=HTTP_TIMEOUT
                )
                ...
                if response.status_code == 200:
                    ...
                    if "searchResultsDTO" in data and "processorResults" in data["searchResultsDTO"]:
                        results = data["searchResultsDTO"]["processorResults"]
                        if results:
                            processor_id = results[0].get("id")
                            processor_name = results[0].get("name")
                            
                            if processor_id:
                                # Asignar a la variable global correspondiente
                                globals()[proc['var']] = processor_id
                                print(f"  ✅ {proc['name']} -> {processor_name} ({processor_id})")
                                encontrados += 1
                                
                                # Si es PutDatabaseRecord, también asignar a PUT_DB_PROCESSOR_ID
                                if proc['name'] == "PutDatabaseRecord" and "PUT_DB_PROCESSOR_ID" not in [p['var'] for p in procesadores_claves]:
                                    globals()["PUT_DB_PROCESSOR_ID"] = processor_id
                                    print(f"  ✅ PUT_DB_PROCESSOR_ID -> {processor_id}")
                                    encontrados += 1
                            else:
                                print(f"  ⚠️ {proc['name']} - No se encontró ID")
                        else:
                            print(f"  ⚠️ {proc['name']} - No se encontraron resultados")
                else:
                    print(f"  ❌ {proc['name']} - Error en búsqueda: {response.status_code}")
                    
            except Exception as e:
                print(f"  ❌ Error buscando {proc['name']}: {str(e)}")
                
        # Si no se encontró PUT_DB_PROCESSOR_ID y sí se encontró PUT_DATABASE_RECORD_ID, usar ese valor
        if PUT_DATABASE_RECORD_ID and not PUT_DB_PROCESSOR_ID:
            PUT_DB_PROCESSOR_ID = PUT_DATABASE_RECORD_ID
            print(f"  ✅ PUT_DB_PROCESSOR_ID -> {PUT_DB_PROCESSOR_ID} (copiado de PUT_DATABASE_RECORD_ID)")
            encontrados += 1
            
        # Mostrar resumen
        print(f"\n✅ Se encontraron {encontrados}/{len(procesadores_claves) + 1} procesadores")
        
        # Verificar IDs de grupos (usando valores hardcodeados por ahora)
        PRODUCER_GROUP_ID = "4dcc0bed-9061-3a4c-0000-c375f77615eb" 
        CONSUMER_GROUP_ID = "723a3709-dabb-3fae-0000-2892da95b9a6"
        print(f"  📋 PRODUCER_GROUP_ID: {PRODUCER_GROUP_ID}")
        print(f"  📋 CONSUMER_GROUP_ID: {CONSUMER_GROUP_ID}")
        ...
        print(f"❌ Error general reinicializando IDs: {str(e)}")
        return False

def verificar_estado_detallado_nifi():
    """
    Realiza un diagnóstico completo del estado de NiFi usando múltiples endpoints
    para determinar la raíz de los problemas de comunicación
    """
    try:
        print("🔍 REALIZANDO DIAGNÓSTICO COMPLETO DEL ESTADO DE NIFI")
        print("=" * 70)
        ...
        try:
            print("\n1️⃣ Verificando estado general del sistema...")
            response = requests.get(
                f"{MINI_PROXY_URL}/api/system/status",
                timeout=HTTP_TIMEOUT
            )
            
            if response.status_code == 200:
                data = response.json()
                resultados["estado_general"] = data.get("status", "desconocido")
                resultados["detalles"]["system_status"] = data
                
                if "auth" in data:
                    resultados["autenticacion"] = "ok" if data["auth"].get("working", False) else "error"
                
                if "connectivity" in data:
                    resultados["conexion"] = "ok" if data["connectivity"].get("success", False) else "error"
                    
                print(f"  ✓ Estado general: {resultados['estado_general']}")
                print(f"  ✓ Autenticación: {resultados['autenticacion']}")
                print(f"  ✓ Conectividad: {resultados['conexion']}")
            else:
        ...
        try:
            print("\n2️⃣ Descubriendo endpoints disponibles...")
            
            # Lista de endpoints a verificar
            endpoints = [
                "/api/system/status",
                "/api/flow/search-results",
                "/api/processors",
                "/api/clean-all-queues",
                "/diagnostics",
                "/health",
                "/api/flow/process-groups/root"
            ]
            
            for endpoint in endpoints:
                try:
                    url = f"{MINI_PROXY_URL}{endpoint}"
                    response = requests.get(url, timeout=5)  # Timeout corto para no demorar demasiado
                    
                    disponible = response.status_code != 404
                    estado = f"✓ {response.status_code}" if disponible else f"✗ {response.status_code}"
                    print(f"  {estado} - {endpoint}")
                    
                    if disponible:
                        resultados["endpoints_disponibles"].append(endpoint)
                except Exception as e:
                    print(f"  ✗ Error con endpoint {endpoint}: {str(e)}")
        except Exception as e:
            print(f"  ✗ Error verificando endpoints: {str(e)}")
        
        # 3. Verificar endpoints específicos de procesadores
        try:
            print("\n3️⃣ Verificando procesador de prueba (QueryDatabaseTable)...")
            proceso_prueba_id = QUERY_DB_TABLE_PROCESSOR_ID
            
            # Intentar obtener detalles por endpoint directo
            response = requests.get(
                f"{MINI_PROXY_URL}/api/processors/{proceso_prueba_id}",
                timeout=HTTP_TIMEOUT
            )
            
            if response.status_code == 200:
                print(f"  ✓ Endpoint directo funciona: {response.status_code}")
                data = response.json()
                if "component" in data and "state" in data["component"]:
                    print(f"  ✓ Estado real del procesador: {data['component']['state']}")
            else:
                print(f"  ✗ Endpoint directo error: {response.status_code}")
            
            # Intentar búsqueda por tipo
            search_response = requests.get(
                f"{MINI_PROXY_URL}/api/flow/search-results?q=QueryDatabaseTable",
                timeout=HTTP_TIMEOUT
            )
            
            if search_response.status_code == 200:
                print(f"  ✓ Endpoint de búsqueda funciona: {search_response.status_code}")
                search_data = search_response.json()
                if "searchResultsDTO" in search_data and "processorResults" in search_data["searchResultsDTO"]:
                    results = search_data["searchResultsDTO"]["processorResults"]
                    ...
                    if len(results) > 0:
                        found_processor = results[0]
                        print(f"  ✓ Procesador encontrado: {found_processor.get('name')} -> {found_processor.get('id')}")
                        resultados["procesadores"][found_processor.get('name')] = found_processor.get('id')
                else:
                    print("  ✗ No se encontraron resultados en la búsqueda")
            else:
                print(f"  ✗ Endpoint de búsqueda error: {search_response.status_code}")
        except Exception as e:
            print(f"  ✗ Error verificando procesador: {str(e)}")
        
        # 4. Verificar explícitamente cada procesador importante
        procesadores_claves = [
            {"id": QUERY_DB_TABLE_PROCESSOR_ID, "name": "QueryDatabaseTable"},
            {"id": CONSUME_KAFKA_PROCESSOR_ID, "name": "ConsumeKafka"},
            {"id": PUBLISH_KAFKA_ID, "name": "PublishKafka"},
        ...
        print("\n4️⃣ Verificando estado individual de procesadores clave...")
        for proc in procesadores_claves:
            try:
                # Primero intentar con endpoint directo
                response = requests.get(
                    f"{MINI_PROXY_URL}/api/processors/{proc['id']}",
                    timeout=5  # Timeout corto
                )
                
                if response.status_code == 200:
                    data = response.json()
                    estado = data.get('component', {}).get('state', 'DESCONOCIDO')
                    print(f"  ✓ {proc['name']} ({proc['id']}): {estado}")
                    resultados["procesadores"][proc['name']] = {
                        "id": proc['id'],
                        "estado": estado,
                        "encontrado": True
                    }
                else:
                    print(f"  ✗ {proc['name']} ({proc['id']}): Error {response.status_code}")
                    
                    # Intentar búsqueda alternativa
                    search = requests.get(
                        f"{MINI_PROXY_URL}/api/flow/search-results?q={proc['name']}",
                        timeout=5
                    )
                    if search.status_code == 200:
                        ...
                        if "searchResultsDTO" in search_data and "processorResults" in search_data["searchResultsDTO"]:
                            results = search_data["searchResultsDTO"]["processorResults"]
                            if results:
                                print(f"  ✓ {proc['name']} encontrado mediante búsqueda: {results[0].get('id')}")
                                resultados["procesadores"][proc['name']] = {
                                    "id": results[0].get('id'),
                                    "estado": results[0].get('state', 'DESCONOCIDO'),
                                    "encontrado": True,
                                    "id_diferente": results[0].get('id') != proc['id']
                                }
                ...
                print(f"  ✗ Error verificando {proc['name']}: {str(e)}")
                resultados["procesadores"][proc['name']] = {
                    "id": proc['id'],
                    "estado": "ERROR",
                    "error": str(e),
        ...
        # 5. Verificar operaciones de inicio/detención
        print("\n5️⃣ Probando operación básica (detener procesador)...")
        # Seleccionar un procesador que esté disponible de las verificaciones anteriores
        proc_disponible = None
        for name, info in resultados["procesadores"].items():
            if isinstance(info, dict) and info.get("encontrado", False):
                proc_disponible = {"id": info["id"], "name": name}
                break
        
        if proc_disponible:
            print(f"  ✓ Usando procesador disponible: {proc_disponible['name']} ({proc_disponible['id']})")
            
            try:
                # Obtener estado actual y revisión
                status_resp = requests.get(
                    f"{MINI_PROXY_URL}/api/processors/{proc_disponible['id']}",
                    timeout=HTTP_TIMEOUT
                )
                
                if status_resp.status_code == 200:
                    status_data = status_resp.json()
                    current_state = status_data.get("component", {}).get("state", "UNKNOWN")
                    revision = status_data.get("revision", {})
                    
                    print(f"  ✓ Estado actual: {current_state}")
                    
                    # Intentar la operación contraria a su estado actual
                    ...
                    operation_resp = requests.put(
                        f"{MINI_PROXY_URL}/api/processors/{proc_disponible['id']}/run-status",
                        json=update_data,
                        timeout=HTTP_TIMEOUT
                    )
                    
                    if operation_resp.status_code in [200, 201, 202]:
                        print(f"  ✓ Operación exitosa: cambio a estado {target_state}")
                        resultados["operacion_basica"] = "ok"
                    else:
                        print(f"  ✗ Error en operación: {operation_resp.status_code}")
                        resultados["operacion_basica"] = f"error:{operation_resp.status_code}"
                else:
                    print(f"  ✗ Error obteniendo estado: {status_resp.status_code}")
                    resultados["operacion_basica"] = f"error:{status_resp.status_code}"
            except Exception as e:
                print(f"  ✗ Error en operación: {str(e)}")
                resultados["operacion_basica"] = f"error:{str(e)}"
        else:
            print("  ✗ No se encontró ningún procesador disponible para probar operación")
            resultados["operacion_basica"] = "no_disponible"
        ...
        return {"estado_general": "error", "error": str(e)}
...
def registrar_log(pg_hook, nivel, mensaje, alimentador=None, fecha=None, hora=None, detalles=None):
    ...
    try:
        ...
        detalles_completos = {
            ...
            'fuente': 'potencia_integrador_anual',
            'hora': hora
        }
...
def recuperar_datos_mes_faltantes(mes=None, **kwargs):
    ...
    try:
        ...
        configurar_topic_procesador(CONSUME_KAFKA_PROCESSOR_ID, "Topics", topic_name)
...
def procesar_alimentador_para_mes_mejorado(pg_hook, productor, alimentador, mes, year=YEAR):
    ...
    try:
        ...
        for dato in datos_originales:
            ...
            for out in outliers:
                if (out['fecha'] == registro['fecha'] and 
                    str(out['hora']) == str(registro['hora'])):
                    es_outlier = True
                    break
            
            # Si es cero, marcar como standby para meses < 6
            if es_cero and mes < 6:
                ...
                continue # Skip el resto del procesamiento
            
            # Aplicar depuración si es necesario
            valores_antiguos = None
            
            if necesita_depuracion and (es_outlier or (es_cero and mes >= 6)):
                valores_antiguos = {
            ...
            if registro['potencia_activa'] > 0 or registro['potencia_reactiva'] > 0:
                # Formato para Kafka con solo los campos necesarios y asegurando tipos correctos
                dia_semana_calculado = calcular_dia_semana(registro['fecha'])
                mensaje_kafka = {
                    'fecha': registro['fecha'],
                    'hora': registro['hora'],
                    'alimentador': registro['alimentador'],
                    'dia_semana': dia_semana_calculado,  # Siempre calculado correctamente
                ...
                try:
                    ...
                    if registros_enviados_kafka % 100 == 0:
...
def iniciar_procesadores_productor():
    ...
    try:
        ...
        if gestor_nifi:
            print("▶️ Iniciando SOLO procesadores del productor mediante gestor...")
            return gestor_nifi.iniciar_procesadores_productor()
        
        # Si no hay gestor, usar enfoque antiguo
        producer_processors = [
            {"id": QUERY_DB_TABLE_PROCESSOR_ID, "name": "QueryDatabaseTable"},
            {"id": CONVERT_RECORD_ID, "name": "ConvertRecord"},
            {"id": SPLIT_JSON_ID, "name": "SplitJson"},
...
def iniciar_procesadores_consumidor():
    ...
    try:
        ...
        if gestor_nifi:
            print("▶️ Iniciando procesadores del consumidor mediante gestor...")
            return gestor_nifi.iniciar_procesadores_consumidor()
...
def verificar_si_necesita_depuracion(ti, mes, nombre_mes):
    ...
    """
    # Verificar primero si los datos ya existen y son completos
...
with dag:
    ...
    for mes, nombre_mes in meses_info:
        grupo_mes = crear_taskgroup_mes(mes, nombre_mes)
        grupos_meses.append(grupo_mes)
    
    # Verificación de completitud anual
    ...
    # Decisión sobre ejecución de calidad de datos
    ...
    finalizar_procesamiento = DummyOperator(
        task_id="finalizar_procesamiento_completo",
        trigger_rule="all_done"
    )

    # Grupo de tareas para calidad de datos
    with TaskGroup(group_id="calidad_datos") as calidad_datos_group:
        ...
        imputar_faltantes = PythonOperator(
            ...
            python_callable=imputar_faltantes,
        ...
        # Secuencia dentro del grupo de calidad
    ...
    # Tarea inicial que conecta al primer mes
    iniciar_proceso = DummyOperator(
        task_id="iniciar_proceso",
        trigger_rule="all_success"
    )

    # Configurar la secuencia principal con punto de inicio explícito
    verificar_tabla_original >> setup_connections >> verificar_disponibilidad >> verificar_proxy >> iniciar_proceso >> grupos_meses[0]
    
    # Encadenar los meses en secuencia estricta
    for i in range(len(grupos_meses)-1):
        grupos_meses[i] >> grupos_meses[i+1]
    
    # Secuencia final con dependencias explícitas
    grupos_meses[-1] >> verificar_completitud
    verificar_completitud >> decidir_calidad
    decidir_calidad >> [calidad_datos_group, limpiar_tmp]
    calidad_datos_group >> limpiar_tmp
    limpiar_tmp >> restaurar >> finalizar_procesamiento